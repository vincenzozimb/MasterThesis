...
let 
    ...
    # error calculation
    a1 = vec(mean(energy_sample[:,2:end] .* energy_sample[:,1:end-1], dims=1))
    b1 = vec(mean(energy_sample[:,2:end], dims=1)) .* vec(mean(energy_sample[:,1:end-1], dims=1))
    
    a2 = vec(mean(energy_sample[:,3:end] .* energy_sample[:,1:end-2], dims=1))
    b2 = vec(mean(energy_sample[:,3:end], dims=1)) .* vec(mean(energy_sample[:,1:end-2], dims=1))
